# 프로젝트 bloblow 회고 Daily

### 공통
- 실력을 키워야 더 솔직하게 커뮤니케이션 할 수 있을 것.
...
### 10.30
#### 개발자 사고에 더 익숙해지기 (Action item: 내가 만든 기능들에 필요했던 세부 기능 쪼개 보고 복기)
...
- URI, end point란?
- API 만드는 건 request, response 둘 만한 것들을 만들면 됨. 단순 로직 흐름상으로만 구현하면 되면 굳이 API로 팔 필요 없음.
...
- React Query 상태 관리 라이브러리 사용할거라면 꼭 Zustand도 써야 하나? 2가지 상태 관리 라이브러리를 사용하는 것이 처음 들었을 때는 낯설었음. 안쓰고 React Query만으로 클라이언트, 서버 상태를 모두 관리할 수 없나? => 결국 각각 사용목적과 강점이 다름
	1. 각각 사용 목적과 강점이 다름.
...
- 오늘 비로소 API 의 'I(Interface)'의 의미를 조금이나마 체감했다. 그전에는 왜 인터페이스라는 용어를 쓰는지 감이 안왔다. 프론트엔드는 UI를 보고 코드를 짜고, 백엔드는 API를 보고 코드를 짜서 그런가? 역시 계속 보고 사용해봐야 느낌이 온다.
- 아이패드에 REST API 공부한거도 정리하기. '각 상태를 마치 변수 처럼 관리한다는 포인트'
- 아직 초기 세팅이 낯선 것 같다. 어떤 순서대로 할지 감 잡아두자.
- 테일윈드를 devDependencies로 해도 되는 이유
	- 정적이라 빌드 때만 필요하다?
- 개인 프로젝트 때 초반 시간 아낄 수 있을 방법을 생각해보자.
...
- 백엔드는 어느정도 정석이 정해진 것 가은데 프론트엔드는 정답이 없고 많이 유연한 느낌.
...
- 남은 시간 19:20 부터 23:30 까지 4시간 남았다. 제한 시간 안에 모달 UI 끝내고, 테스트 후 수정사항도 업데이트 해야 함. 내가 빨리 기능 쳐야 내가 할 수 있는 범위가 늘어난다. 집중하자. 나에게 이로운 환경을 찾아가서 하자. 소파에서 하자.

### 11.08
- 어제 `SubkeywordModal`(추가/제외 키워드 편집 모달) 구현을 5시간 가량 투입했다.
	- 더 빨리 할 수 없었을까?
	- 규경님이 짜신 코드를 거의 그대로 사용했다. 빨리 하는 건 중요해서 어쩔 수 없고, 회사 가서도 비슷하게 하겠지만 알고 써야 한다.
...
- 현재 에셋 뭐가 있는지 한 판 볼 수 있으면 좋겠다. 변수명 만으로는 알기 힘듦. 코드 상으로도 알 수 없고.

- 다른 서비스 참고하면서 스타일링 참고하는 것도 좋은 방법이겠다.
	- ![rt-bloblow-ref-ohouse](/src/image/rt-bloblow-ref-ohouse.png)
- 기록) 유저가 많을 때는 에러를 어떻게 확인?
- yml 파일 형식은 굳이 왜 있나, pug도.. html, css, js로 충분하지않나
- 회고한거 기록하자. API 만들기 사전에 협의했으면 좋았을 것을. 컨트롤러를 어떤 단위로 묶을지.
- 잘 하고 싶은 이유. 내가 아이디어 내는거 다 내가 할 줄 알아야. 내 말에 책임.
- 기록) 컴포넌트라 리랜더링 된다고 상태가 초기화 되는게 아니다. 초기화는 처음 한 번 실행되고, 그 다음 리렌더링 되도 상태는 계속 기억된다. 헷갈리지 말 것. 상태는 언제 사라지나?
...
### 11.19
- 정렬할 때 가장 중요한 건 동점이 없도록 하는 것. 정렬 기준 여러개 사용할 수도 있어야.
